...
\begin{document}
  ...
  \begin{IEEEeqnarray}{rCl}
    \exp
    \begin{bmatrix}
      0 & \displaystyle\int_0^t ds + \displaystyle\frac{\sigma}{\mu + \sigma^2 / 2} dW_s\\
      0 & \displaystyle\int_0^t\left(\mu + \sigma^2 / 2\right) ds + \sigma dW_s
    \end{bmatrix}
    & = &
    \begin{bmatrix}
      1 & Y_t\\
    ...
    \end{bmatrix}
  \end{IEEEeqnarray}
  From which we determine that in $\mathfrak{h}_2$ the process is just standard Brownian
  motion. As we will see next this raises the prospect of a deep connection between the
  adjoint derivative of the exponential map and stochastic differential equations.
