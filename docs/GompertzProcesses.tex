...
\begin{document}
  ...
  By the application of It\^o calculus to the independent future increments of the carrier
  Brownian motion process $W_t$, conditioning the future on the past generates a measure
  space that is the product of two $\sigma$-finite measure spaces. We cannot say the same
  for the bridged conditioning of the past of $X_s$ on the future of $Y_t$. The conditional
  probability of $\operatorname{\mathbb{P}}\left[ X_s \right\rVert\left. Y_t \right]$ for
  times $s<t$ irreducibly mixes the time $s$ and accelerations $X_s$, so that the measure
  space of the conditional probability is not the product measure space of two
  $\sigma$-finite measure spaces. As such we cannot apply the Fubini-Tonelli theorem to
  integrate processes conditioned on the future of the integral. The crux of the failure is
  that the bridged conditioning $X_s$ on it's future integral $Y_t$ introduces a reciprocal
  constraint between the size of the excursions of $X_s$ and the duration of the excursions
  of $X_s$, because by the construction of the integral the area under the excursion of
  $X_s$ must be less than it's future integral $Y_t$.
  ...
  \begin{IEEEeqnarray}{rCl}
    ...
    \int_0^x \int_0^y
    \frac{
      \operatorname{\mathbb{P}}\left[ X_s = u, Y_s = v \right]
  ...
  Consider the sets of successively closer mean value theorem approximations of $Y_t$ the
  integral of $X_t$, a continuous stochastic process, constrained to $Y_t>y$, with the
  Riemann sum indexed by $n \ge 2$:
  \begin{IEEEeqnarray}{rCl}
    A_n
    & = &
    \left\lbrace \frac{1}{n-1} \sum_{m=0}^{n-2} X_{\frac{n}{m}t} \vee X_{\frac{n+1}{m}t} > \frac{y}{t} \right\rbrace
  \end{IEEEeqnarray}
  ...
  sets. To consider the joint probability of $X_t > x$ and $Y_t > y$ we further constrain
  the approximations $A_n$:
  \begin{IEEEeqnarray}{rCl}
    B_n
    & = &
    \left\lbrace \frac{1}{n-1} \sum_{m=0}^{n-2} X_{\frac{n}{m}t} \vee X_{\frac{n+1}{m}t} > \frac{y}{t}, X_{\frac{n-1}{n}t} > \frac{n-1}{n}x \right\rbrace
  \end{IEEEeqnarray}
  The sets push the constrained approximations of $Y_t$ into the future so that the stopped
  indicator processes of:
  \begin{IEEEeqnarray}{rCl}
    A_{s \uparrow t}
    & = &
    \begin{cases}
      \operatorname{\mathbb{I}}\left[A_{\left\lceil \frac{t}{t-s} \right\rceil}\right] & s < t,\\
      \operatorname{\mathbb{I}}\left[A_\infty \right] & \text{else}
    \end{cases}\\
    B_{s \uparrow t}
    & = &
    \begin{cases}
      \operatorname{\mathbb{I}}\left[B_{\left\lceil \frac{t}{t-s} \right\rceil}\right] & s < t,\\
      \operatorname{\mathbb{I}}\left[B_\infty \right] & \text{else}
    \end{cases}\\
  \end{IEEEeqnarray}
  are adapted to the filtration $\mathscr{F}_s \subset \mathscr{F}_t$ of the carrier
  Brownian motion $W_s$ before $W_t$. Liberally invoking the continuity of $X_t$ and the
  strict increase of it's sums we define the integral process $Y_t>y$ by requiring that the
  approximations eventually hold in the limit inferior sense:
  \begin{IEEEeqnarray}{rCl}
    \liminf A_n
    & = &
  ...
  \end{IEEEeqnarray}
  The limit superior and limit inferior of the sets differ only by a negligible set in 
  $\mathscr{F}_t$:
  \begin{IEEEeqnarray}{rCl}
    \limsup A_n - \liminf A_n
    & = &
    \left\lbrace X_t = \frac{y}{t}, \int_0^t X_u du = y \right\rbrace\\
    & \subset &
    \left\lbrace X_t = \frac{y}{t} \right\rbrace\\
    \limsup B_n - \liminf B_n
    & = &
    \begin{cases}
      \left\lbrace X_t = \frac{y}{t}, \int_0^t X_u du = y \right\rbrace & x = \frac{y}{t},\\
      \emptyset & \text{else}
    \end{cases}\\
    & \subset &
    \left\lbrace X_t = \frac{y}{t} \right\rbrace\\
  ...
  nonetheless equivalent in probability in $\mathscr{F}_t$. Note that point sets have
  negligible measure because we are implicitly working within an integral of the actual
  probability measure and not the Radon-Nikodym probability density. For clarity I have used
  Riemann sums; if one were tackle this argument rigorously they would need to replace the
  indexing with positive integers with a net of finite partitions of time. Through
  the countable construction of the limit inferiors we reach the future constraints by
  carefully pruning the paths in the of present $\mathscr{F}_s \subset \mathscr{F}_t$
  without ever actually peaking into the future of $\mathscr{F}_t$ to condition on a
  future event. Thus the marginal probability $\operatorname{\mathbb{P}}\left[Y_t \right]$
  and joint probability $\operatorname{\mathbb{P}}\left[ X_t , Y_t \right]$ exist and are
  finite.
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      ...
      \begin{IEEEeqnarray}{rCl}
        ...
        \operatorname{\mathbb{E}}\left[X_u^{m+n}\right]
        \operatorname{\mathbb{E}}\left[ X_{t-u}^m \right]
  ...
  \begin{IEEEeqnarray}{rCl}
    ...
    \frac{ \operatorname{\mathbb{P}}\left[ T_{n+m} = t \right]}
    { \operatorname{\mathbb{P}}\left[ T_n=t \right]}\\\nonumber\\
  ...
  \begin{IEEEeqnarray}{rCl}
    \IEEEeqnarraymulticol{3}{l}{
      \operatorname{\mathbb{P}}\left[ Y_{t_{n+1}} - Y_{t_n} = y_{n+1} - y_n \right\rVert\left. Y_{t_n} = y_n, \dots , Y_{t_0} = y_0 \right]
    }\nonumber\\
    & \qquad = &
    \operatorname{\mathbb{P}}\left[X_{t_n-t_{n-1}} Y_{t_{n+1}-t_n} = \frac{y_{n+1}-y_n}{y_n-y_{n-1}} Y_{t_n-t_{n-1}}\right]\\
    & \qquad = &
